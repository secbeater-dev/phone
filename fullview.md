...
`dataset-store.js` 提供 dataset/target/kind/time 複合索引、users/targets、temporary strings、磁碟排序索引、磁碟彙總和摘要快取。非時間排序、搜尋、統計都在 Worker 分批掃描；熱點不持有全時刻陣列，明細另行分頁。multi 電話統計只算通聯，legacy 電話統計保留所有記錄種類。IMEI 以識別碼索引分頁。`hotspots` 查詢同時搜尋地址及任一通聯時間，以有界批次建立磁碟結果集合，保留符合熱點的完整次數。熱點卡百分比分母為目前時段記錄數；縣市視窗的分母為所有地址出現次數，與舊版一致。
...
- 使用提醒：本地運行說明、請私訊「作者」連到 Telegram（不顯示網址）、強制重載最新版（等同 Ctrl+F5；只更新網頁，不會清除備註、主題等本機設定），以及車輛辨識／蝦殼分析的同源圖片連結（`object-fit: contain`）。左上角品牌連到 `https://secbeater.com/`。
...
- 2026-09-11：恢復多門號功能前的初始介面與既有分析版型；以來源格式控制目標區、網路入口與 PDF，保留 Worker／IndexedDB 有界查詢、分卷匯出及格式切換的資料採用保護。新增來源格式往返、legacy 全記錄摘要與排行、完整異常日期提示、地址／時間熱點搜尋與舊版百分比、用戶欄位去重及 IMEI 索引排序。清除或採用新批次同時清空舊投單欄位，避免殘留上一批電話。新增台哥大 XML 展平 XLSX parser 與合成續行回歸；補上初始／有資料畫面比對及私密資料夾逐檔／整批相容性驗證。
...
- 2026-09-10：首次發布後辨識 CDN 對部署前 404 的短期快取，發布資產版本升為 `20260910-multi-phone-v2`，並增加整個 BrowserContext（含 Worker）的同源 GET／無資料提交測試。
- 2026-09-10：正式站 v2 的 12 個執行資產 SHA-256 與本機一致，7 項瀏覽器回歸通過。CDN 注入的外部 beacon 與 inline challenge 由 CSP 阻擋；測試區分被 CSP 阻止的嘗試與實際網路回應，並確認沒有 challenge iframe。CDN 自身的同源 speculation GET 不含通聯資料。
- 2026-09-10：正式站另通過 10 萬／20 萬筆合成通聯的匯入與大型 XLSX 分卷／取消測試；效能數字與硬體驗證限制記於 `docs/verification-2026-09-10.md`，未記錄私密來源筆數或識別資訊。
...
- 2026-08-27：使用提醒圖卡改為完整顯示不裁切，Telegram 只把「作者」做成連結。
