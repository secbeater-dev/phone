...
async function handle(op, payload, signal, progress) {
  ...
  if (op === 'encodeAttachment') {
    ...
    if (payload.format === 'xlsx') {
      if (!xlsxLoaded) { importScripts('./vendor/exceljs.min.js'); xlsxLoaded = true; }
    ...
    if (!pdfLoaded) { importScripts('./vendor/pdf-lib.min.js', './vendor/fontkit.umd.min.js', './vendor/open-huninn-data.js'); pdfLoaded = true; }
